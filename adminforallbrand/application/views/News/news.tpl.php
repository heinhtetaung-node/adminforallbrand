...
<div id="content" class="bodylayout" style="display:none"><!-- second common-->
	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4" ng-init="brandidsjson = <?php echo htmlspecialchars(json_encode($brandids,JSON_NUMERIC_CHECK)); ?>">
			<h1 class="page-title txt-color-blueDark"><i class="fa-fw fa fa-home"></i> News Control Panel <span style="color: #4C4F53 !important;  margin: 12px 0px 28px;  letter-spacing:-1px; font-size: 24px; margin: 10px 0px; text-transform:capitalize"  ng-show="b.brand_id==para_brand_id" ng-repeat="b in brandidsjson">{{b.brand_name}}</span></h1>
		</div>	
	</div>
				...
				<div class="jarviswidget jarviswidget-color-blueDark jarviswidget-sortable" id="wid-id-1" data-widget-deletebutton="false" data-widget-editbutton="false">
					...
					<div>
						...
						<div class="widget-body no-padding">
							...
							<table  id="tbl" ng-init="getdatas('tbl_new')" class="table table-hover" width="100%">
								<thead>			                
									<tr>
										<th class="hideinsmall idrow" ng-click="sortField = 'new_id'; reverse = !reverse"> <a href="">ID</a></th>
										<th class="" ng-click="sortField = 'brand_id'; reverse = !reverse"> <a href="">Brand</a></th>
										<th ng-click="sortField = 'new_title'; reverse = !reverse"><i class="fa fa-user text-muted hidden-md hidden-sm hidden-xs"></i> <a href="">Title</th>
								...
								<tbody>
									<!--
									SELECT `new_id`, `language_id`, `brand_id`, `new_title`, `new_description`, `status`, `created_date` FROM `tbl_new` WHERE 1
									-->
									<!-- today edit 19 Jan --><tr id="{{datas.new_id}}" tr-id="{{datas.new_id}}" ng-repeat="datas in filtered = (datas | filter:{brand_id:search2, new_title:search1, new_description:search1} | orderBy : sortField :reverse |  startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit)" ng-hide="para_brand_id !='' && para_brand_id!=datas.brand_id">
									
										<td class="hideinsmall idrow">{{datas.new_id}}</td>
										<td class="capitalized">{{datas.brand_name}}</td>
										<td  class="">{{datas.new_title}}</td>
										...
										<td class="option">
											<a href="" ng-click="prepareedit(datas.new_id)" class="fa fa-edit"></a> &nbsp;
											<a href="" ng-click="deletedata(datas.new_id, 'tbl_new', 'new_id')" class="fa fa-times-circle"></a> 
										</td>
									</tr>
						...
						<div class="widget-body no-padding">
							...
							<form role="form" ng-submit="savedata()" >
								...
								<div class="form-group">
									...
									<select required id="brand_id" placeholder="Select Brand" ng-model="brand_id" class="form-control">
										<option></option>
										<?php foreach($brandids as $b){ ?>
											<option ng-hide="para_brand_id !='' && para_brand_id!=<?php echo $b->brand_id; ?>" value="<?php echo $b->brand_id; ?>"><?php echo $b->brand_name; ?></option>
										<?php } ?>
								...
								<div class="form-group" ng-init="getlanguages('tbl_language')">
								...
								<div class="form-group">
									<label>Title</label>
									<input required ng-model="new_title" type="text" class="form-control" placeholder="Enter ...">
								</div>
								
								<div class="form-group">
									<label>Description</label>
									
									<textarea  required ng-model="new_description" id="editor1" name="editor1" rows="10" cols="80" class="form-control" style="width:100%; min-height:500px;" placeholder="Place some text here"></textarea>                      							
								</div>
								
								<div class="form-group">
									<label>Status : &nbsp; &nbsp; </label>
									
									<input ng-checked="'true'" class="inputradio" type="radio" value="1" ng-model="new_status.selectvalue"> Active &nbsp; &nbsp;  <input class="inputradio"  type="radio" value="0" ng-model="new_status.selectvalue"> Disable
									
								</div>
								
								<input type="hidden"  ng-model="new_id" >
